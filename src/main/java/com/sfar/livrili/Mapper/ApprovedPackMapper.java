...
public class ApprovedPackMapper {
    ...
    public ApprovedPackDto toApprovedPackDto(Pack pack) {
        ...
        Optional<Offer> findAcceptedOffer = pack.getOffers().stream()
                .filter(offer -> offer.getStatus() == OfferStatus.ACCEPTED).findFirst();
        ...
        return ApprovedPackDto.builder()
                ...
                .deliveryGuyName(acceptedOffer.getDeliveryPerson().getFirstName() + " "
                        + acceptedOffer.getDeliveryPerson().getLastName())
