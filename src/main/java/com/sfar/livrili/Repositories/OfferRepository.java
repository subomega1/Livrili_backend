...
public interface OfferRepository extends CrudRepository<Offer, UUID> {
    ...
    Optional<UUID> PackToDeliver(@Param("userId") UUID userId, @Param("packId") UUID packId,
            @Param("offerStatus") OfferStatus offerStatus, @Param("packStatus") PackageStatus packStatus);

    @Query("SELECT o.pack FROM Offer o WHERE o.status = :offerStatus AND o.deliveryPerson.id = :userId and o.pack.status = :packStatus")
    List<Pack> getDeliveredPacks(@Param("userId") UUID userId, @Param("offerStatus") OfferStatus offerStatus,
            @Param("packStatus") PackageStatus packStatus);
