    ...
    public ResponseEntity<Map<String, String>> handleInvalidFormatException(HttpMessageConversionException ex) {
        if (ex.getMessage().contains("Role")) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new HashMap<>(Map.of("Role", "Invalid Role ,must be CLIENT or DELIVERY_PERSON.")));
        } else if (ex.getMessage().contains("Gender")) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new HashMap<>(Map.of("Gender", "Invalid Gender, must be MALE or FEMALE.")));
        } else {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new HashMap<>(Map.of("OfferStatus", "Invalid OfferStatus must be ACCEPTED or DECLINED.")));
        }
    }
